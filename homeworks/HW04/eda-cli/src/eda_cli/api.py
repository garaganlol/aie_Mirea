...
@app.post(
    ...
    tags=["quality"],
    summary="Вернуть полный набор флагов качества по CSV (EDA-ядро)"
)
async def quality_flags_from_csv(file: UploadFile = File(...)):
    """
    Принимает CSV-файл, прогоняет EDA-ядро (summarize_dataset, missing_table, compute_quality_flags)
    и возвращает полный словарь флагов качества в поле "flags".
    """
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
        raise HTTPException(status_code=400, detail="Ожидается CSV-файл (content-type text/csv).")
    ...
    # Приводим все данные к JSON-serializable формату
    flags_serializable = jsonable_encoder(flags_all)
    ...
    print(
        f"[quality-flags-from-csv] filename={file.filename!r} n_rows={n_rows} "
        f"n_cols={n_cols} flags_count={len(flags_serializable)}"
    )

    return {"flags": flags_serializable}