...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # has_high_cardinality_categoricals — флаг, сигнализирующий,
    # что есть категориальные признаки с очень большим числом уникальных значений (нужно определить свой порог).
    ...
    # has_suspicious_id_duplicates  — для числовых колонок проверить долю нулей и выставить флаг, если она превышает выбранный порог.

    id_like_cols = [c for c in summary.columns if "id" in c.name.lower()]
    duplicated_id_columns = []

    for col in id_like_cols:
        if col.unique < summary.n_rows:  # значит есть дубли
            duplicated_id_columns.append(col.name)

    flags["has_suspicious_id_duplicates"] = len(duplicated_id_columns) > 0
    flags["id_duplicate_columns"] = duplicated_id_columns
